...
public class Midi {
     ...
     * "rest(time)" denotes the time separating every pair of note-on or note-off events.
     ...
     * We clearHistory(), then start a note with midi frequency 60, then rest 30 ms, 
     * then start a note with midi frequency 65, then rest 100 ms, then release the first note, 
     * then rest 30 ms, then release the second note, and then call history().  
     * history() will return this string:
     * "on(60,PIANO) rest(3) on(65,PIANO) rest(10) off(60,PIANO) rest(3) off(65,PIANO)"
     * 
     ...
     * returned by history().  The note entry is preceded by a rest(time) entry 
     * when appropriate.
     * 
    ...
    private void log(int note, midi.Instrument instr, boolean isNoteOn) {
        ...
        if(prevEventTime > 0) {
        	...
        	stringBuilder.append("rest("+timeDif+") ");
        }
        prevEventTime = time;
        if(isNoteOn) {
